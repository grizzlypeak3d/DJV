...
namespace djv
{
    namespace ui
    {
        ...
        struct ViewMissingFramesWidget::Private
        {
            std::shared_ptr<ftk::CheckBox> enabledCheckBox;
            std::shared_ptr<ftk::IntEditSlider> sizeEdit;
            std::shared_ptr<ftk::IntEditSlider> lineWidthEdit;
        ...
        void ViewMissingFramesWidget::_init(
            const std::shared_ptr<ftk::Context>& context,
            const std::shared_ptr<models::ViewportModel>& viewportModel,
            const std::shared_ptr<ftk::IWidget>& parent)
        {
            ftk::IWidget::_init(context, "djv::app::ViewMissingFramesWidget", parent);
            FTK_P();

            p.enabledCheckBox = ftk::CheckBox::create(context);
            p.enabledCheckBox->setHStretch(ftk::Stretch::Expanding);
            p.enabledCheckBox->setTooltip(
                "Mark a picture that stands in for a frame the sequence does\n"
                "not have, which happens with a render still in progress.\n"
                "\n"
                "Brackets are drawn inside the corners of the image, and the\n"
                "heads up display time says which frame is being held.\n"
                "\n"
                "Applies to the hold and black missing frame settings. The\n"
                "others either show nothing or leave no missing frames to\n"
                "mark.");
            ftk::setScreenshotTag(p.enabledCheckBox, "View.MissingFrames.Enabled");

            p.sizeEdit = ftk::IntEditSlider::create(context);
            p.sizeEdit->setRange(10, 200);
            ftk::setScreenshotTag(p.sizeEdit, "View.MissingFrames.Size");

            p.lineWidthEdit = ftk::IntEditSlider::create(context);
            p.lineWidthEdit->setRange(1, 20);
            ftk::setScreenshotTag(p.lineWidthEdit, "View.MissingFrames.LineWidth");

            p.colorSwatch = ftk::ColorSwatch::create(context);
            p.colorSwatch->setEditable(true);
            p.colorSwatch->setHAlign(ftk::HAlign::Left);
            ftk::setScreenshotTag(p.colorSwatch, "View.MissingFrames.Color");
            ...
            p.optionsObservers = ftk::Observer<tl::ForegroundOptions>::create(
                ...
                {
                    FTK_P();
                    p.enabledCheckBox->setChecked(value.missingIndicator.enabled);
                    p.sizeEdit->setValue(value.missingIndicator.size);
                    p.lineWidthEdit->setValue(value.missingIndicator.width);
                    p.colorSwatch->setColor(value.missingIndicator.color);
                });
            ...
            p.sizeEdit->setCallback(
                ...
                {
                    auto options = viewportModel->getForegroundOptions();
                    options.missingIndicator.size = value;
                    viewportModel->setForegroundOptions(options);
                });
        ...
        ViewMissingFramesWidget::ViewMissingFramesWidget() :
            _p(new Private)
        {}
        ...
        std::shared_ptr<ViewMissingFramesWidget> ViewMissingFramesWidget::create(
            const std::shared_ptr<ftk::Context>& context,
            const std::shared_ptr<models::ViewportModel>& viewportModel,
            const std::shared_ptr<IWidget>& parent)
        {
            auto out = std::shared_ptr<ViewMissingFramesWidget>(new ViewMissingFramesWidget);
            out->_init(context, viewportModel, parent);
            return out;
        }

        ftk::Size2I ViewMissingFramesWidget::getSizeHint() const
        {
            return _p->layout->getSizeHint();
        }

        void ViewMissingFramesWidget::setGeometry(const ftk::Box2I& value)
        {
            IWidget::setGeometry(value);
            _p->layout->setGeometry(value);
        }
