...
namespace djv
{
    ...
    {
        //! Speed popup.
        class DJV_UI_API_TYPE SpeedPopup : public ftk::IWidgetPopup
        {
        ...
        public:
            DJV_UI_API virtual ~SpeedPopup();

            DJV_UI_API static std::shared_ptr<SpeedPopup> create(
                const std::shared_ptr<ftk::Context>&,
                const std::shared_ptr<ftk::DoubleModel>&,
                double defaultSpeed,
                const std::shared_ptr<IWidget>& parent = nullptr);

            DJV_UI_API void setCallback(const std::function<void(double)>&);

            DJV_UI_API void open(
