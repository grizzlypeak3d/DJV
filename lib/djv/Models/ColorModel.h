...
namespace djv
{
    ...
    {
        //! Color model.
        class DJV_API_TYPE ColorModel : public std::enable_shared_from_this<ColorModel>
        {
        ...
        public:
            ...
            //! Create a new model.
            DJV_API static std::shared_ptr<ColorModel> create(
                const std::shared_ptr<ftk::Context>&,
                const std::shared_ptr<ftk::Settings>&);

            //! Get the OpenColorIO options.
            DJV_API const tl::OCIOOptions& getOCIOOptions() const;

            //! Observe the OpenColorIO options.
            DJV_API std::shared_ptr<ftk::IObservable<tl::OCIOOptions> > observeOCIOOptions() const;

            //! Set the OpenColorIO options.
            DJV_API void setOCIOOptions(const tl::OCIOOptions&);
            ...
            //! them.
            DJV_API std::shared_ptr<ftk::IObservable<tl::OCIOOptions> > observeResolvedOCIOOptions() const;
            ...
            DJV_API void setActiveFiles(const std::vector<std::pair<std::string, ftk::ImageTags> >&);
            ...
            DJV_API std::string resolveInput(
            ...
            DJV_API std::shared_ptr<ftk::IObservable<std::vector<std::string> > > observeResolvedInputs() const;

            //! Observe the resolved input color space and where it came
            //! from, for display; e.g., "sRGB (extension)". Empty when
            //! nothing is resolved.
            DJV_API std::shared_ptr<ftk::IObservable<std::string> > observeResolvedInput() const;

            //! Get the file name extension color spaces.
            DJV_API const std::map<std::string, std::string>& getExtColorSpaces() const;

            //! Observe the file name extension color spaces.
            DJV_API std::shared_ptr<ftk::IObservable<std::map<std::string, std::string> > > observeExtColorSpaces() const;
            ...
            DJV_API void setExtColorSpaces(const std::map<std::string, std::string>&);

            //! Get the LUT options.
            DJV_API const tl::LUTOptions& getLUTOptions() const;

            //! Observe the LUT options.
            DJV_API std::shared_ptr<ftk::IObservable<tl::LUTOptions> > observeLUTOptions() const;

            //! Set the LUT options.
            DJV_API void setLUTOptions(const tl::LUTOptions&);
