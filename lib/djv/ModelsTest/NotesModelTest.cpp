...
namespace djv
{
    namespace models_tests
    {
        ...
        void NotesModelTest::_notes()
        {
            ...
            FTK_CHECK(model->getNotes().empty());
            ...
            FTK_CHECK(2 == model->getNotes().size());
            FTK_CHECK(2 == count);
            const auto& notes = model->getNotes();
            FTK_CHECK(!notes[0].id.empty());
            FTK_CHECK(!notes[0].created.empty());
            FTK_CHECK(notes[0].id != notes[1].id);
            FTK_CHECK(128.0 == notes[0].time->value());

            // Removing by identifier leaves the other note alone.
            const std::string id = notes[0].id;
            model->remove(id);
            FTK_CHECK(1 == model->getNotes().size());
            FTK_CHECK("Second note." == model->getNotes()[0].text);
